...
# 嘗試導入 OBS_UDP，如果未編譯則提供降級或佔位
try:
    # 嘗試直接導入 (如果在 capture 目錄下運行)
    try:
        import OBS_UDP
        OBS_UDP_Manager = OBS_UDP.OBS_UDP_Manager
        HAS_UDP = True
        print("[Capture] OBS_UDP module loaded successfully (direct).")
    except ImportError:
        # 嘗試作為包導入 (如果在根目錄運行)
        from .OBS_UDP import OBS_UDP_Manager
        HAS_UDP = True
        print("[Capture] OBS_UDP module loaded successfully (package).")
except ImportError as e:
    HAS_UDP = False
    print(f"[Capture] OBS_UDP module import failed: {e}")
    print("[Capture] UDP mode will be unavailable. Please ensure OBS_UDP.cp312-win_amd64.pyd exists in 'capture/' folder.")
...
class CaptureService:
    ...
    def connect_udp(self, ip, port):
        ...
        if not HAS_UDP:
            return False, "UDP module not loaded (OBS_UDP.pyd missing or failed to import)"
        ...
        try:
            ...
            # OBS_UDP_Manager.connect(ip, port, target_fps=0, max_decode_workers=4, enable_async=True)
            success = self.udp_manager.connect(self._ip, self._port, target_fps=0, max_decode_workers=4, enable_async=True)
            if success:
